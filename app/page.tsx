        ...
        <div className="mx-auto grid max-w-6xl gap-16 px-6 py-24 lg:grid-cols-[1.15fr_0.85fr]">
          <div className="space-y-8">
            <div className="section-heading">
              ...
              <h2 className="section-heading__text">About the Academy</h2>
              <span className="section-heading__line" aria-hidden="true" />
            </div>
            <p className="font-body text-lg leading-relaxed text-ivory/90">
              Hawthorne Dominion House Academy is an elite private institution devoted to the preservation of order, excellence, and
              honor. Our faculty, fellows, and prefects uphold a code of scholarly conduct that balances classical tradition with
              contemporary research. Students are selected not merely for aptitude, but for their commitment to service and their
              readiness to shoulder responsibility.
            </p>
            <p className="font-body text-lg leading-relaxed text-ivory/80">
              Through ceremonial Houses and rigorous tutorial systems, every scholar is mentored to ascend beyond expectation. Our
              community is one of quiet resolve, precision, and refined courtesy.
            </p>
          </div>
          <div className="panel space-y-6 backdrop-blur">
            <h3 className="font-display text-xl font-semibold tracking-[0.35em] text-accent-crimson">
              Dominion Standards
            </h3>
            <ul className="space-y-4 font-body text-base leading-relaxed text-ivory/85">
              <li>
      ...
      <section id="academics" className="fade-in section-vignette border-b border-white/15 bg-ink">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <div className="max-w-4xl space-y-8">
            <div className="section-heading">
              <span className="section-heading__line" aria-hidden="true" />
              <h2 className="section-heading__text">Academics</h2>
              <span className="section-heading__line" aria-hidden="true" />
            </div>
            <p className="font-body text-lg leading-relaxed text-ivory/90">
          ...
          <div className="mt-14 grid gap-8 md:grid-cols-3">
            {academies.map((item) => (
              <article key={item.title} className="panel h-full space-y-4">
                <h3 className="font-display text-lg font-semibold tracking-[0.35em] text-accent-crimson">
                  {item.title}
                </h3>
                <p className="font-body text-base leading-relaxed text-ivory/85">{item.description}</p>
              </article>
            ))}
          </div>
        </div>
      ...
      <section id="admissions" className="fade-in-delayed section-vignette border-b border-white/15 bg-parchmentDark/60">
        <div className="mx-auto grid max-w-6xl gap-16 px-6 py-24 lg:grid-cols-[1.2fr_0.8fr]">
          <div className="space-y-8">
            <div className="section-heading">
              <span className="section-heading__line" aria-hidden="true" />
              <h2 className="section-heading__text">Admissions</h2>
              <span className="section-heading__line" aria-hidden="true" />
            </div>
            <p className="font-body text-lg leading-relaxed text-ivory/90">
              Entry to Hawthorne Dominion House Academy is accorded to those whose scholarly record and personal comportment reflect the
              academy’s ideals. Invitations to the winter examinations are extended upon review of recommendations and essays of intent.
            </p>
            <p className="font-body text-lg leading-relaxed text-ivory/80">
              Prospective scholars should prepare dossiers attesting to academic rigor, leadership in service, and mastery of the arts or
              sciences. Interviews with the Faculty Council reaffirm character and commitment before induction.
            </p>
          </div>
          <aside className="panel space-y-4 backdrop-blur">
            ...
            <ul className="space-y-3 font-body text-base leading-relaxed text-ivory/85">
              <li>
      ...
      <section id="faculty" className="fade-in section-vignette border-b border-white/15 bg-ink">
        <div className="mx-auto max-w-6xl px-6 py-24">
          <div className="max-w-4xl space-y-8">
            <div className="section-heading">
              <span className="section-heading__line" aria-hidden="true" />
              <h2 className="section-heading__text">Faculty</h2>
              <span className="section-heading__line" aria-hidden="true" />
            </div>
            <p className="font-body text-lg leading-relaxed text-ivory/90">
              Guided by deans and fellows of exceptional pedigree, the faculty of Hawthorne Dominion House Academy safeguard the integrity
              of every discipline. Their mentorship ensures that the academy’s ethos is carried forward in each cohort.
            </p>
          </div>
          <div className="mt-14 grid gap-8 md:grid-cols-3">
            {facultyHighlights.map((faculty) => (
              <article key={faculty.name} className="panel h-full space-y-4">
                <h3 className="font-display text-lg font-semibold tracking-[0.35em] text-ivory">
                  {faculty.name}
                </h3>
                <p className="font-display text-xs tracking-[0.6em] text-ivory/65" style={{ fontVariant: "small-caps" }}>
                  {faculty.role}
                </p>
                <p className="font-body text-base leading-relaxed text-ivory/85">{faculty.note}</p>
              </article>
            ))}
          </div>
        </div>
